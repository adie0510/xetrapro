...
spec = importlib.util.spec_from_file_location("s3", "/Users/adarsh/xetraPro/xetrapro/xetra/common/s3.py")
...
spec3 = importlib.util.spec_from_file_location("meta_process", "/Users/adarsh/xetraPro/xetrapro/xetra/common/meta_process.py")
...
class TestMetaProcessMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_bucket_meta = foo.S3BucketConnector(self.s3_access_key,
                                                self.s3_secret_key,
                                                self.s3_endpoint_url,
                                                self.s3_bucket_name)
        self.dates = [(datetime.today().date() - timedelta(days=day))\
            .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(8)]
    ...
    def test_update_meta_file_no_meta_file(self):
        ...
        meta_key = 'meta.csv'
        # Method execution
        foo3.MetaProcess.update_meta_file(date_list_exp, meta_key, self.s3_bucket_meta)
        # Read meta file
        data = self.s3_bucket.Object(key=meta_key).get().get('Body').read().decode('utf-8')
        out_buffer = StringIO(data)
        df_meta_result = pd.read_csv(out_buffer)
        date_list_result = list(df_meta_result[foo1.MetaProcessFormat.META_SOURCE_DATE_COL.value])
        proc_date_list_result = list(
            pd.to_datetime(df_meta_result[foo1.MetaProcessFormat.META_PROCESS_COL.value])\
            .dt.date
            )
    ...
    def test_update_meta_file_empty_date_list(self):
        ...
        with self.assertLogs() as logm:
            result = foo3.MetaProcess.update_meta_file(date_list, meta_key, self.s3_bucket_meta)
    ...
    def test_update_meta_file_meta_file_ok(self):
        ...
        meta_key = 'meta.csv'
        meta_content = (
          f'{foo1.MetaProcessFormat.META_SOURCE_DATE_COL.value},'
          f'{foo1.MetaProcessFormat.META_PROCESS_COL.value}\n'
          f'{date_list_old[0]},'
          f'{datetime.today().strftime(foo1.MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}\n'
          f'{date_list_old[1]},'
          f'{datetime.today().strftime(foo1.MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}'
        )
        self.s3_bucket.put_object(Body=meta_content, Key=meta_key)
        # Method execution
        foo3.MetaProcess.update_meta_file(date_list_new, meta_key, self.s3_bucket_meta)
        # Read meta file
        data = self.s3_bucket.Object(key=meta_key).get().get('Body').read().decode('utf-8')
        out_buffer = StringIO(data)
        df_meta_result = pd.read_csv(out_buffer)
        date_list_result = list(df_meta_result[
            foo1.MetaProcessFormat.META_SOURCE_DATE_COL.value])
        proc_date_list_result = list(pd.to_datetime(df_meta_result[
            foo1.MetaProcessFormat.META_PROCESS_COL.value])\
                .dt.date)
        # Test after method execution
        self.assertEqual(date_list_exp, date_list_result)
        self.assertEqual(proc_date_list_exp, proc_date_list_result)
    ...
    def test_update_meta_file_meta_file_wrong(self):
        ...
        meta_key = 'meta.csv'
        meta_content = (
          f'wrong_column,{foo1.MetaProcessFormat.META_PROCESS_COL.value}\n'
          f'{date_list_old[0]},'
          f'{datetime.today().strftime(foo1.MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}\n'
          f'{date_list_old[1]},'
          f'{datetime.today().strftime(foo1.MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)}'
        ...
        self.s3_bucket.put_object(Body=meta_content, Key=meta_key)
        # Method execution
        with self.assertRaises(foo2.WrongMetaFileException):
            foo3.MetaProcess.update_meta_file(date_list_new, meta_key, self.s3_bucket_meta)
        # Cleanup after test
        self.s3_bucket.delete_objects(
            Delete={
    ...
    def test_return_date_list_no_meta_file(self):
        ...
        date_list_exp = [
            (datetime.today().date() - timedelta(days=day))\
                .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(4)
            ]
        min_date_exp = (datetime.today().date() - timedelta(days=2))\
            .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value)
        # Test init
        first_date = min_date_exp
        meta_key = 'meta.csv'
        # Method execution
        min_date_return, date_list_return = foo3.MetaProcess.return_date_list(first_date, meta_key,
                                                                         self.s3_bucket_meta)
        # Test after method execution
    ...
    def test_return_date_list_meta_file_ok(self):
        ...
        date_list_exp = [
          [(datetime.today().date() - timedelta(days=day))\
              .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(3)],
          [(datetime.today().date() - timedelta(days=day))\
              .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(4)],
          [(datetime.today().date() - timedelta(days=day))\
              .strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(9)]
          ]
        # Test init
        meta_key = 'meta.csv'
        meta_content = (
    ...
    def test_return_date_list_meta_file_wrong(self):
        ...
        with self.assertRaises(KeyError):
            foo3.MetaProcess.return_date_list(first_date, meta_key, self.s3_bucket_meta)
        # Cleanup after test
        self.s3_bucket.delete_objects(
            Delete={
                'Objects': [
