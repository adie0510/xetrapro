...
spec2 = importlib.util.spec_from_file_location("xetra_transformer", "/Users/adarsh/xetraPro/xetrapro/xetra/transformers/xetra_transformer.py")
...
class IntTestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_endpoint_url = 'https://us-east-1.console.aws.amazon.com'
        self.s3_bucket_name_src = 'xetra-int-test-src2'
        self.s3_bucket_name_trg = 'xetra-int-test-trg2'
        ...
        self.s3_bucket_src = foo.S3BucketConnector(self.s3_access_key,
                                                self.s3_secret_key,
                                                self.s3_endpoint_url,
                                                self.s3_bucket_name_src)
        self.s3_bucket_trg = foo.S3BucketConnector(self.s3_access_key,
                                                self.s3_secret_key,
                                                self.s3_endpoint_url,
                                                self.s3_bucket_name_trg)
        # Creating a list of dates
        self.dates = [(datetime.today().date() - timedelta(days=day))\
            .strftime(foo3.MetaProcessFormat.META_DATE_FORMAT.value) for day in range(8)]
        ...
        self.source_config = foo2.XetraSourceConfig(**conf_dict_src)
        self.target_config = foo2.XetraTargetConfig(**conf_dict_trg)
    ...
    def test_int_etl_report1_no_metafile(self):
        ...
        # Method execution
        xetra_etl = foo2.XetraETL(self.s3_bucket_src, self.s3_bucket_trg,
