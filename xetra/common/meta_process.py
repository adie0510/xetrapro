...
class MetaProcess():
    ...
    @staticmethod
    def update_meta_file(extract_date_list: list, meta_key: str, s3_bucket_meta: foo.S3BucketConnector):
        """
        ...
        df_new[foo1.MetaProcessFormat.META_SOURCE_DATE_COL.value] = extract_date_list
        # Filling the processed column
        df_new[foo1.MetaProcessFormat.META_PROCESS_COL.value] = \
          datetime.today().strftime(foo1.MetaProcessFormat.META_PROCESS_DATE_FORMAT.value)
        try:
            ...
            if collections.Counter(df_old.columns) != collections.Counter(df_new.columns):
                raise foo2.WrongMetaFileException
            df_all = pd.concat([df_old, df_new])
        except s3_bucket_meta.session.client('s3').exceptions.NoSuchKey:
            # No meta file exists -> only the new data is used
            df_all = df_new
        # Writing to S3
        s3_bucket_meta.write_df_to_s3(df_all, meta_key, foo1.MetaProcessFormat.META_FILE_FORMAT.value)
        return True

    @staticmethod
    def return_date_list(first_date: str, meta_key: str, s3_bucket_meta: foo.S3BucketConnector):
        """
        ...
        start = datetime.strptime(first_date,
                                  foo1.MetaProcessFormat.META_DATE_FORMAT.value)\
        ...
        try:
            ...
            src_dates = set(pd.to_datetime(
              df_meta[foo1.MetaProcessFormat.META_SOURCE_DATE_COL.value]
              ).dt.date)
            ...
            if dates_missing:
                ...
                return_dates = [
                    date.strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) \
            ...
            return_dates = [
              (start + timedelta(days=x)).strftime(foo1.MetaProcessFormat.META_DATE_FORMAT.value) \
